...
import { AuthService, User, JwtAuthGuard } from '@app/auth';
...
export class AuthController {
  ...
  public async signUp(@Body() signUpUser: SignUpDto, @Req() request: RequestWithUser<UserModel>): Promise<UserModel> {
    ...
    const { cookie: refreshTokenCookie, token: refreshToken } = await this.authService.generateJwtRefreshTokenCookie({
      userId,
      username,
      email
    });
  ...
  public async refreshToken(@Request() { user, res }: RequestWithUser<UserModel>) {
    const { id: userId, username, email } = user;
