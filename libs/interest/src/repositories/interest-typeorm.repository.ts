...
export class InterestTypeOrmRepository implements InterestRepository {
  ...
  public async bulkCreate(entities: InterestModel[]): Promise<InterestEntity[]> {
  ...
  public find(findManyOptions: Partial<InterestEntity>): Promise<[] | InterestEntity[]> {
  ...
  public findOne(findOptions: Partial<InterestEntity>): Promise<InterestEntity> {
    return this.interestRepository.findOne({
      where: findOptions
    });
  }
  ...
  public async getInterestRate(amount: number, month: number): Promise<number> {
    ...
    if (!interest) {
      throw new InternalServerErrorException('Interest rate not found for the given amount and month.');
    }

    return interest.rate;
  }

  public async updateById(id: number, entity: Partial<InterestEntity>): Promise<InterestEntity> {
    await this.interestRepository.update({ id }, entity);
    return this.findById(id);
  }

  public deleteById(id: number): Promise<boolean> {
    return this.interestRepository.delete({ id }).then((result) => !!result.affected);
  }

  public softDeleteById(id: number): Promise<boolean> {
    return this.interestRepository.softDelete({ id }).then((result) => !!result.affected);
  }
}
