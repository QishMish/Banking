...
export class SchedulerService {
  ...
  public async accountScheduler() {
    ...
    this.logger.log(
      `Retrieved saving accounts: ${JSON.stringify(accountsResult)}`,
    );

    await Promise.all([
      this.updateSavingAccounts(
        accountsMap.get(SavingAccountType.TERM_DEPOSIT),
        SavingAccountType.TERM_DEPOSIT,
      ),
      this.updateSavingAccounts(
        accountsMap.get(SavingAccountType.FIXED_DEPOSIT),
        SavingAccountType.FIXED_DEPOSIT,
      ),
    ]);

    this.logger.log(
      `Saving accounts updated:  ${JSON.stringify(accountsResult)}`,
    );

    return void 0;
  }

  public async updateSavingAccounts(
    ...
    const promises = accounts.map(async (acc) => {
      ...
      const newInterestAmount = (currentBalance * currentInterestRate).toFixed(
        2,
      );

      const currentDate = new Date();

      const monthIncrementValue =
        type == SavingAccountType.FIXED_DEPOSIT ? params.term : 1;
      ...
      await this.dbTransactionService.executeTransaction(async () => {
        ...
        const updatedParams = await this.accountParamsService.update(
          params.id,
          {
            ...
            nextInterestPayDate: new Date(
              currentDate.setMonth(
                currentDate.getMonth() + monthIncrementValue,
              ),
            ),
          },
        );
