...
export class AccountSubscriber implements EntitySubscriberInterface<AccountEntity> {
  ...
  afterUpdate(event: UpdateEvent<AccountEntity>): void | Promise<AccountEntity> {
    try {
      const { status, balance } = event.entity;

      const balanceUpdated = event.updatedColumns.find((value) => value.propertyName === 'balance');
      const statusUpdated = event.updatedColumns.find((value) => value.propertyName === 'status');
